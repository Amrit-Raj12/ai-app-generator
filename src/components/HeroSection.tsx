...
import CursorTrailV2 from "@/components/CursorTrailV2";

function HeroSection() {

    const { theme } = useTheme()
    const [isDark, setIsDark] = useState(false)
    const [sparkleColors, setSparklesColor] = useState(['#FF0000', '#00FF00', '#0000FF'])
    ...
    useEffect(() => {
        ...
        if (isDark) {
            setSparklesColor(['#D3D3D3', '#E0E0E0', '#F0F0F0', '#CCCCCC'])
        } else {
            setSparklesColor(['#8B5CF6', '#A855F7', '#C084FC', '#DDD6FE'])
        }
    ...
    return (
        <div className="custom-cursor">
            ...
            <div className="finisher-header" style={{ position: 'absolute', top: 0, left: 0, inset: 0, zIndex: -1, height: '100vh' }}></div>
            {/* <CustomCursor /> */}
            <CursorTrailV2
                enabled={true}
                sparkleCount={4}
